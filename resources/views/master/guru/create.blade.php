...
@section('page_title', 'Formulir Guru & Tenaga Pendidik Baru')
...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            ...
            <h3 class="text-xl font-extrabold text-slate-900 tracking-tight">Formulir Pendidik Baru</h3>
            <p class="text-xs sm:text-sm text-slate-500 mt-0.5">Lengkapi identitas guru atau staf pengajar SD Islam Bina Insan Mandiri.</p>
        </div>
            ...
            <div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-5 mt-5">
                    ...
                    <div class="sm:col-span-2">
                        ...
                        <input type="text" name="nama_guru" id="nama_guru" value="{{ old('nama_guru') }}" required placeholder="Contoh: Ustadz Ahmad Zaki, S.Pd.I" class="w-full px-4 py-2.5 bg-slate-50 border @error('nama_guru') border-rose-300 ring-1 ring-rose-300 @else border-slate-300 @enderror rounded-xl text-sm focus:outline-hidden focus:ring-2 focus:ring-emerald-500 focus:bg-white transition-all">
                        @error('nama_guru')
                    ...
                    <div>
                        ...
                        <input type="text" name="nip" id="nip" value="{{ old('nip') }}" placeholder="Contoh: 198501152010011005" class="w-full px-4 py-2.5 bg-slate-50 border @error('nip') border-rose-300 ring-1 ring-rose-300 @else border-slate-300 @enderror rounded-xl text-sm focus:outline-hidden focus:ring-2 focus:ring-emerald-500 focus:bg-white transition-all">
                        @error('nip')
                            <p class="text-xs text-rose-600 mt-1">{{ $message }}</p>
                        @enderror
                    </div>
            ...
            <div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-5 mt-5">
                    ...
                    <div>
                        ...
                        <input type="email" name="email" id="email" value="{{ old('email') }}" placeholder="Contoh: ustadz.ahmad@sdi-bim.sch.id" class="w-full px-4 py-2.5 bg-slate-50 border @error('email') border-rose-300 ring-1 ring-rose-300 @else border-slate-300 @enderror rounded-xl text-sm focus:outline-hidden focus:ring-2 focus:ring-emerald-500 focus:bg-white transition-all">
                        @error('email')
                            <p class="text-xs text-rose-600 mt-1">{{ $message }}</p>
                        @enderror
                    </div>
