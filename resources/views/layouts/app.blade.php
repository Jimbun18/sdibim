        ...
        <aside id="main-sidebar" class="fixed inset-y-0 left-0 z-50 w-72 bg-slate-900 text-slate-300 flex flex-col transition-transform duration-300 ease-in-out -translate-x-full lg:translate-x-0 lg:static lg:inset-auto border-r border-slate-800 shadow-2xl">
            ...
            <div class="h-20 flex items-center justify-between px-6 bg-slate-950 border-b border-slate-800/80">
                <div class="flex items-center space-x-3.5">
                    <div class="w-11 h-11 rounded-xl bg-gradient-to-br from-emerald-500 to-teal-700 flex items-center justify-center text-white shadow-lg shadow-emerald-900/40 ring-2 ring-emerald-400/30">
            ...
            <nav class="flex-1 px-4 py-5 space-y-1.5 overflow-y-auto custom-scrollbar">
                ...
                <!-- 1. Modul Dashboard -->
                <a href="{{ url('/') }}" class="flex items-center px-3.5 py-2.5 rounded-xl text-sm font-medium transition-all group {{ request()->is('/') || request()->is('dashboard') ? 'bg-emerald-600 text-white shadow-lg shadow-emerald-900/30 font-semibold' : 'text-slate-300 hover:bg-slate-800 hover:text-white' }}">
                    <svg class="w-5 h-5 mr-3 transition-colors {{ request()->is('/') || request()->is('dashboard') ? 'text-white' : 'text-slate-400 group-hover:text-emerald-400' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <a href="{{ url('/keuangan/spp') }}" class="flex items-center justify-between px-3.5 py-2.5 rounded-xl text-sm font-medium transition-all group {{ request()->is('keuangan*') ? 'bg-emerald-600 text-white shadow-lg shadow-emerald-900/30 font-semibold' : 'text-slate-300 hover:bg-slate-800 hover:text-white' }}">
                    <div class="flex items-center">
                        <svg class="w-5 h-5 mr-3 transition-colors {{ request()->is('keuangan*') ? 'text-white' : 'text-slate-400 group-hover:text-emerald-400' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <a href="{{ url('/absensi') }}" class="flex items-center justify-between px-3.5 py-2.5 rounded-xl text-sm font-medium transition-all group {{ request()->is('absensi*') ? 'bg-emerald-600 text-white shadow-lg shadow-emerald-900/30 font-semibold' : 'text-slate-300 hover:bg-slate-800 hover:text-white' }}">
                    <div class="flex items-center">
                        <svg class="w-5 h-5 mr-3 transition-colors {{ request()->is('absensi*') ? 'text-white' : 'text-slate-400 group-hover:text-emerald-400' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <a href="{{ url('/akademik/nilai') }}" class="flex items-center px-3.5 py-2.5 rounded-xl text-sm font-medium transition-all group {{ request()->is('akademik*') ? 'bg-emerald-600 text-white shadow-lg shadow-emerald-900/30 font-semibold' : 'text-slate-300 hover:bg-slate-800 hover:text-white' }}">
                    <svg class="w-5 h-5 mr-3 transition-colors {{ request()->is('akademik*') ? 'text-white' : 'text-slate-400 group-hover:text-emerald-400' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <a href="{{ url('/mapel') }}" class="flex items-center px-3.5 py-2.5 rounded-xl text-sm font-medium transition-all group {{ request()->is('mapel*') ? 'bg-emerald-600 text-white shadow-lg shadow-emerald-900/30 font-semibold' : 'text-slate-300 hover:bg-slate-800 hover:text-white' }}">
                    <svg class="w-5 h-5 mr-3 transition-colors {{ request()->is('mapel*') ? 'text-white' : 'text-slate-400 group-hover:text-emerald-400' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253"></path>
                    </svg>
            ...
            <div class="p-4 bg-slate-950 border-t border-slate-800/80">
                <div class="flex items-center space-x-3">
                    ...
                    <div class="flex-1 min-w-0">
                        <p class="text-xs font-semibold text-white truncate">Tata Usaha & Admin</p>
                        <p class="text-[11px] text-slate-400 truncate">admin@sdi-bim.sch.id</p>
                    </div>
                    <span class="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-medium bg-emerald-500/20 text-emerald-300 border border-emerald-500/30">
                        Online
                    </span>
                </div>
            </div>
                ...
                <div class="flex items-center space-x-4">
                    ...
                    <div>
                        <div class="flex items-center gap-2 text-xs font-medium text-slate-400">
                            ...
                            <span>&bull;</span>
                            <span class="text-emerald-600 font-semibold">SD Islam Bina Insan Mandiri</span>
                        </div>
                        <h2 class="text-lg font-bold text-slate-900 leading-tight">@yield('page_title', 'Data Siswa')</h2>
                    </div>
                </div>

                <div class="flex items-center space-x-3 sm:space-x-4">
                    <!-- Quick Date Badge -->
                    <div class="hidden md:flex items-center space-x-2 text-xs font-medium text-slate-600 bg-slate-100 px-3 py-1.5 rounded-lg border border-slate-200">
                    ...
                    <!-- Islamic Greeting -->
                    <div class="hidden sm:inline-block px-3 py-1 text-xs font-semibold bg-emerald-50 text-emerald-700 border border-emerald-200/60 rounded-lg">
                        بِسْمِ اللَّهِ الرَّحْمَٰنِ الرَّحِيمِ
                    </div>

                    <!-- Quick Notifications / Help -->
                    <button class="p-2 text-slate-400 hover:text-emerald-600 hover:bg-slate-100 rounded-lg transition-colors relative" title="Pusat Bantuan SIM">
